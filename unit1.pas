...
type typerecherche = (sclient, sanimal);
type recherches = set of typerecherche;

type aliment = packed record
    nom : string;
    source: string;
    indications: string;
...
type aliments = array of aliment;

type race = packed record
    nom: string;
    facteur: double;
end;

type races = array of race;

type activite = packed record
    nom: string;
    facteur: double;
end;

type activites = array of activite;
...
const val_clinique='Calcul de ration';
...
type ages = (fa_year, fa_month, fa_day);
type set_ages = set of ages;
...
SQL.Add('SELECT * FROM CLIENTS');
Open;
if not EOF then showmessage(FieldByName('NOM').Asstring);
Connection.Disconnect;
end;
